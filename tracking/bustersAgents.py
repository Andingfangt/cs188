...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            mostLikelyPosition = max(zip(dist.values(), dist.keys()))[1]
            mostLikelyPositions.append(mostLikelyPosition)
        
        # find the closest ghost position
        closestGhostPosition = min(zip([self.distancer.getDistance(pacmanPosition, p) for p in mostLikelyPositions], mostLikelyPositions))[1]

        #chooses an action that brings Pacman closest to the closest ghost (according to mazeDistance!).
        chooseAction = min(zip([self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), closestGhostPosition)  for a in legal], legal))[1]
